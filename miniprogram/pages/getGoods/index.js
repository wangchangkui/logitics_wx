...
Page({
  ...
  check() {
    ...
    else if (that.data.address.length == 0 || that.data.address == "" || that.data.getCode.length == 0 || that.data.getCode == "" || that.data.userName == "" || that.data.userName.length == 0) 
    {
      wx.showToast({
        title: '订单数据不完整',
        icon: "error"
      })
    }else if(that.data.goodsAddress == null || that.data.goodsAddress == undefined || that.data.goodsAddress == ""){
      wx.showToast({
        title: '未填写任务取货地址',
        icon: "error"
      })
      ...
      // 满足条件的时候 提交订单
      wx.request({
        url: app.url.url+'order/createOrder',
        method:'POST',
        header:{
        ...
        data:{
          ...
          "goodsAddress":that.data.goodsAddress  
        },
        success:res=>{
          console.log(res)
  ...
  onSelect(option) {
    this.setData({
      ...
      userName: option.detail.username
    })
  },
  ...
  onLoad: function (options) {
    ...
    let that = this;
    wx.request({
      url: app.url.url+'user/addressList/' + users.userId,
      method: "GET",
      success: res => {
        if (res.data.code == 200000) {
          let datas = new Array();
